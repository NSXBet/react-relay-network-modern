...
import type { Middleware, MiddlewareNextFn, FetchOpts } from '../definition';
...
interface BatchInternalOpts {
  batchTimeout: number;
  batchUrl:
    | string
    | Promise<string>
    | ((requestList: RequestWrapper[]) => string | Promise<string>);
  singleton: { batcher?: Batcher };
  maxBatchSize: number;
...
export default function batchMiddleware(options?: BatchMiddlewareOpts): Middleware {
  ...
  const singleton: { batcher?: Batcher } = {};
...
function prepareNewBatcher(next: MiddlewareNextFn, opts: BatchInternalOpts): Batcher {
  ...
  if (requestList.length === 1) {
    ...
    if (headersOrThunk) {
      const headers = await (isFunction(headersOrThunk)
        ? (headersOrThunk as any)(batchRequest)
        : headersOrThunk);
    ...
    try {
      ...
      batchResponse.json.forEach((payload: any, index: number) => {
        if (!payload) return;
...
function finalizeCanceled(requestList: RequestWrapper[], error: Error): void {
  requestList.forEach((request) => request.completeErr(error));
}

// check that server returns responses for all requests
function finalizeUncompleted(requestList: RequestWrapper[]): void {
