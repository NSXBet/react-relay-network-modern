...
export type AbortFn = (msg?: string | null) => any;
...
interface RetriableRequestOpts {
  req: RelayRequestAny;
...
async function makeRetriableRequest(
  o: RetriableRequestOpts,
  delay: number = 0,
  attempt: number = 0,
  lastError: Error | null = null
): Promise<RelayResponse> {
  const makeRetry = async (prevError: Error): Promise<RelayResponse> => {
  ...
  const makeRequest = async (): Promise<RelayResponse> => {
    try {
...
export function delayedExecution<T>(
  ...
  let abort: AbortFn = noopFn;
  let forceExec: () => any = noopFn;
